                ...
                >
                  {isEdit && (  // Conditionally hide the image when isEdit is true
                    <img
                      src={tab.src}
                      alt={tab.label}
                      style={{ width: "1rem", height: "1rem", marginRight: "15px" }}
                    />
                  )}
            ...
            {teamMembers.map((member, index) => (
              <tr key={index} className="border-b">
                ...
                <td className="p-2">
                  <div
                    className={classNames(
                      "text-600 flex items-center justify-center p-2 border rounded-full",
                      {
                        "text-green-500 border-green-500": member.status === "Active",
                        "text-orange-500 border-orange-500": member.status === "Pending set up",
                        "text-red-500 border-red-500": member.status === "Deactivated",
                      }
                    )}
                    style={{
                      minWidth: "120px",
