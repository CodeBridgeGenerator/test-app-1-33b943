...
import seederData from "./customerInvoice.seeder.json";
...
const CustomerInvoiceSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("customerInvoice").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(CustomerInvoiceSeederDialogComponent);
