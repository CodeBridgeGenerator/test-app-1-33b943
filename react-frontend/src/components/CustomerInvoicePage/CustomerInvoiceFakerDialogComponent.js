...
import customerInvoiceFakerFactory from "./customerInvoiceFakerFactory";
...
const CustomerInvoiceFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = customerInvoiceFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("customerInvoice").create(elem));
...
export default connect(mapState, mapDispatch)(CustomerInvoiceFakerDialogComponent);
